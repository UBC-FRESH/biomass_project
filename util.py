...
def compile_scenario(fm):
    ...
    biomass = [fm.compile_product(period, 'totvol_st', acode='harvest') for period in fm.periods]
    bark = [fm.compile_product(period, 'totvol_bk', acode='harvest') for period in fm.periods]
    branches = [fm.compile_product(period, 'totvol_br', acode='harvest') for period in fm.periods]
    foliage = [fm.compile_product(period, 'totvol_fl', acode='harvest') for period in fm.periods]
...
def plot_scenario(df):
    ...
    ax[0, 0].bar(df.period, df.oha)
    ax[0, 0].set_ylim(0, None)
    ax[0, 0].set_title('Harvested area (ha)')
    ax[0, 0].set_xlabel('period')
    ax[0, 1].bar(df.period, df.ohv)
    ax[0, 1].set_ylim(0, None)
    ax[0, 1].set_title('Harvested volume (m3)')
    ax[0, 1].set_xlabel('period')
    ax[0, 2].bar(df.period, df.ogs)
    ax[0, 2].set_ylim(0, None)
    ax[0, 2].set_title('Growing Stock (m3)')
    ax[0, 2].set_xlabel('period')
    ax[1, 0].bar(df.period, df.biomass)
    ax[1, 0].set_ylim(0, None)
    ax[1, 0].set_title('Biomass (tonne)')
    ax[1, 0].set_xlabel('period')
    ax[1, 1].bar(df.period, df.bark)
    ax[1, 1].set_ylim(0, None)
    ax[1, 1].set_title('Bark (tonne)')
    ax[1, 1].set_xlabel('period')
    ax[1, 2].bar(df.period, df.branches)
    ax[1, 2].set_ylim(0, None)
    ax[1, 2].set_title('Branches (tonne)')
    ax[1, 2].set_xlabel('period')
    ax[1, 3].bar(df.period, df.foliage)
    ax[1, 3].set_ylim(0, None)
    ax[1, 3].set_title('Foliage (tonne)')
    ax[1, 3].set_xlabel('period')
    # Hide the subplot
    ax[0, 3].axis('off')
    return fig, ax
